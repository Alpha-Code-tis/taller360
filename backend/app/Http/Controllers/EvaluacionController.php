    ...
    public function saveEvaluation(Request $request)
{
    ...
    try {
        ...
        foreach ($request->tareas as $tareaData) {
            ...
            // Encontrar la tarea y validar su existencia
            ...
            // Obtener los estudiantes asociados a la tarea
            $estudiantes = EstudianteTarea::where('id_tarea', $tarea->id_tarea)->with('estudiante')->get();
            if ($estudiantes->isEmpty()) {
                return response()->json(['error' => 'No se encontraron estudiantes asociados a la tarea'], 422);
            }

            // Iterar sobre los estudiantes y crear o actualizar el registro en detalle_tarea
            foreach ($estudiantes as $estudianteTarea) {
                // Verificar si ya existe un registro de detalle para la tarea y la semana
                $detalleTarea = DetalleTarea::where('id_tarea', $tarea->id_tarea)
                    ->where('semana_sprint', $request->week)
                    ->first();
