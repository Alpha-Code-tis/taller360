...
const Docentes = () => {
  ...
  const validateForm = () => {
    ...
    if (/\d/.test(formValues.apellidoPaterno)) {
      errors.apellidoPaterno = 'El Apellido Paterno no debe contener números.';
    }
    if (hasThreeConsecutiveEqualLetters(formValues.apellidoPaterno)) {
      errors.apellidoPaterno = 'El Apellido Paterno no debe contener tres letras iguales seguidas.';
    }
    if (hasThreeConsecutiveVowelsOrConsonants(formValues.apellidoPaterno)) {
      errors.apellidoPaterno = 'El Apellido Paterno no debe contener tres vocales o consonantes seguidas.';
    }

    if (/\d/.test(formValues.apellidoMaterno)) {
      errors.apellidoMaterno = 'El Apellido Materno no debe contener números.';
    }
    if (hasThreeConsecutiveEqualLetters(formValues.apellidoMaterno)) {
      errors.apellidoMaterno = 'El Apellido Materno no debe contener tres letras iguales seguidas.';
    }
    if (hasThreeConsecutiveVowelsOrConsonants(formValues.apellidoMaterno)) {
      errors.apellidoMaterno = 'El Apellido Materno no debe contener tres vocales o consonantes seguidas.';
    }
  ...
  const handleSaveNewGroup = async () => {
    ...
    const groupNumber = parseInt(trimmedGroupName, 10);
    ...
    const grupoExiste = grupos.some((grupo) => String(grupo.nro_grupo) === trimmedGroupName);
    if (grupoExiste) {
    ...
    try {
      const response = await axios.post(`${API_URL}grupos`, { nro_grupo: trimmedGroupName});
            ...
            <Form.Group controlId="formNewGroup" className="mb-3">
              ...
              <Form.Control
                ...
                onChange={(e) => {
                  ...
                  if (!/^\d*$/.test(value)) {
                    toast.error('Solo se permiten números en el grupo.');
                    return;
                  }
